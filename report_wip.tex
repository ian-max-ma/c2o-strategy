...
\ins{Completed by teammates.  Input panel: \texttt{outputs/panel\_step3.parquet}
(generated by \texttt{python run\_step2.py}).  Key columns: \texttt{dsi},
\texttt{dtcn}, \texttt{htb\_tier} (A/B/C), \texttt{htb\_flag}.
Tier thresholds and borrow cost schedule are in \texttt{config.py};
the forward-fill logic is in \texttt{step3\_borrow/borrow\_filter.py}.}
